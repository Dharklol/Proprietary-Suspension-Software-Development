...
def _expected_profile_channels() -> dict[str, tuple[float, ...]]:
...
class R25bRuntimeSourceAuditTests(unittest.TestCase):
    def test_exact_frozen_generator_shape_is_accepted(self) -> None:
        audit = audit_channels(_expected_profile_channels())
        self.assertTrue(audit.frozen_generator_description_matches_binary)
        self.assertEqual(audit.total_rows, 3600)
        self.assertEqual(audit.state_count, 36)
        self.assertEqual(audit.rows_per_state_histogram, ((100, 36),))
    ...
    def test_extra_load_plane_fails_profile_match_without_data_repair(self) -> None:
        channels = {name: list(values) for name, values in _expected_profile_channels().items()}
        for slip in _linspace(-12.0, 12.0, 100):
            channels["ET"].append((len(channels["ET"]) + 1) * 0.01)
            channels["FX"].append(0.0)
            channels["FY"].append(-100.0 * slip)
            channels["FZ"].append(890.0)
            channels["IA"].append(0.0)
            channels["MX"].append(0.0)
            channels["MZ"].append(0.0)
            channels["N"].append(470.0)
            channels["P"].append(82.7)
            channels["SA"].append(slip)
            channels["SL"].append(0.0)
            channels["V"].append(40.2)

        audit = audit_channels(channels)
        self.assertFalse(audit.frozen_generator_description_matches_binary)
        self.assertIn(890.0, audit.normal_load_values_n)
    ...
    def test_nonmonotonic_prepeak_state_is_reported_not_repaired(self) -> None:
        channels = _expected_profile_channels()
        mutable = {name: list(values) for name, values in channels.items()}
        state_indices = [
            index
