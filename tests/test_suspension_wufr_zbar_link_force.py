...
AUTH_CONFIG = ZBarLinkForceConfig(rocker_torque_agreement_tolerance_Nm=1.0e-6)
...
def _recover(fixture, state, force):
    return recover_wufr_zbar_physical_link_forces(fixture, state, force, config=AUTH_CONFIG)
...
class WufrZBarPhysicalLinkForceTests(unittest.TestCase):
    ...
    def test_front_coupled_state_recovers_signed_axial_forces_and_torque(self) -> None:
        ...
        for side in (result.left, result.right):
            ...
            self.assertAlmostEqual(
                side.axial_force_N * side.projection_u_dot_n,
                side.elastic_transverse_force_N,
                delta=1.0e-8,
            )
    ...
    def test_rear_asymmetric_state_reproduces_existing_generalized_rocker_torque(self) -> None:
        ...
        self.assertAlmostEqual(
            result.left.physical_rocker_torque_Nm,
            force.generalized_rocker_torque_Nm[0],
            delta=1.0e-6,
        )
        self.assertAlmostEqual(
            result.right.physical_rocker_torque_Nm,
            force.generalized_rocker_torque_Nm[1],
            delta=1.0e-6,
        )
    ...
    def test_force_vectors_are_collinear_with_current_link_axis(self) -> None:
        ...
        for side in (result.left, result.right):
            cross = (
                side.link_axis_blade_to_rocker[1] * side.force_on_rocker_N[2]
                - side.link_axis_blade_to_rocker[2] * side.force_on_rocker_N[1],
                side.link_axis_blade_to_rocker[2] * side.force_on_rocker_N[0]
                - side.link_axis_blade_to_rocker[0] * side.force_on_rocker_N[2],
                side.link_axis_blade_to_rocker[0] * side.force_on_rocker_N[1]
                - side.link_axis_blade_to_rocker[1] * side.force_on_rocker_N[0],
            )
    ...
    def test_upstream_failure_and_source_mismatch_are_fail_closed(self) -> None:
        ...
        result = _recover(fixture, failed_state, good_force)
        ...
        mismatch = _recover(bad_fixture, good_state, good_force)
    ...
    def test_compression_is_preserved_as_negative_when_projection_and_elastic_action_require_it(self) -> None:
        ...
        self.assertAlmostEqual(
            _dot(result.side_force.force_on_blade_N, (1.0, 0.0, 0.0)),
            -25.0,
        )
