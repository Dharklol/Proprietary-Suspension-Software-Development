...
| `MOD-VEH-0003` | Whole-vehicle body/road point transport, wrench assembly, virtual-work generalized-force mapping, rigid four-contact classification, and reviewed WUFR design-intent frame adapter | `prototype_authorized` under `AUTH-VEH-0003`; implementation in PR #46 | Maintain `EQ-VEH-0004` through `0007`, exact frame/origin/application-point contracts, `BENCH-VEH-0003` through `0004`, and source-separated WUFR adapter provenance; separately authorized spring and ARB providers may be composed only by a later equilibrium authorization | Tire/aero/brake/gravity force authority, exact same-session driver z-CG, total/sprung mass force authority, per-corner unsprung allocation, four-corner equilibrium/compatibility, linkage loads, alternate contact modes, installed evidence, and production authority remain separate | Mechanics foundation for later spring/ARB QSS composition and ideal linkage statics |
| `MOD-VEH-0004` | Provider-neutral reduced-coordinate conservative quasi-static equilibrium and active-contact road-reaction recovery | `prototype_authorized` only after `AUTH-VEH-0004` review and merge | Implement `EQ-VEH-0008` through `0010` with explicit compatibility/force-provider contracts and pass `BENCH-VEH-0005` through `0006`; generic/synthetic scope only | WUFR body gravity, sprung/unsprung mass-force allocation, per-corner unsprung force application, tire/aero/brake/powertrain inputs, alternate contact modes, installed evidence, and production authority remain separate; no WUFR road-reaction result until mass authority is reviewed | Generic QSS kernel for later source-bounded WUFR spring/ARB equilibrium composition |
...
| `MOD-SUSP-0003` | WUFR source-bounded rigid push/pull-rod, one-axis rocker, ideal coilover-displacement, and signed local actuation-derivative kinematics | `prototype_authorized` under `AUTH-SUSP-0003`; implementation in PR #44 | Maintain `EQ-SUSP-0009` through `0012`, branch/failure diagnostics, and `BENCH-SUSP-0007` through `0008`; continue supplying reviewed states to separately authorized spring/ARB providers | Damper force, loads/equilibrium, compliance, installed stroke/stops, packaging, correlation, and production authority remain separate | Ideal actuation-state provider for later suspension R&D |
| `MOD-SUSP-0004` | Conservative coil-spring compression, constitutive force, stored energy, tangent stiffness, and generalized-force provider | `prototype_authorized` under `AUTH-SUSP-0004`; implementation merged in PR #48 | Maintain `EQ-SUSP-0013` through `0015`, `BENCH-SUSP-0009` through `0010`, and explicit `ASM-SUSP-0002` provenance | Physical rear spring force-deflection testing, calibrated shock-pot/perch reference evidence, damper/equilibrium, installed/as-built validation, and production authority remain separate; assumption provenance may not be dropped | First conservative suspension-force element for later QSS composition |
| `MOD-SUSP-0005` | Coupled conservative WUFR anti-roll-bar with discrete per-arm blade stiffness, two-arm Z-bar closure, rocker Jacobian/torque, and physical wheel-coordinate generalized force | `prototype_authorized` through staged `AUTH-SUSP-0005` to `AUTH-SUSP-0009`; implementations merged in PR #50, #54, #55, and #56 | Maintain `EQ-SUSP-0016` through `0018`, `BENCH-SUSP-0011` through `0017`, discrete SolidWorks per-arm settings `280/300/400/700/2300 N/mm`, nominal-branch closure, two-step Jacobians, and energy-gradient checks | Whole-vehicle equilibrium/load transfer, tire/contact reactions, installed/as-built correlation, physical blade/system validation, structural loads/stress, compliance/friction/backlash, and production authority remain separate | Conservative ARB force in left/right physical wheel-center vertical coordinates for later QSS equilibrium composition |
...
| `EQ-VEH-0004` through `0007` | Body-fixed point transport, point-force wrench translation, virtual-work generalized force, and rigid flat-road contact admissibility | `prototype_authorized` within `AUTH-VEH-0003`; implementation in PR #46 | Maintain exact frame/origin/application-point contracts, analytical/numerical generalized-force agreement, rigid-contact classifications, and structured failures | No equilibrium solve, negative-load clipping, tire vertical compliance, alternate contact mode, linkage forces, stress, or installed authority; constitutive force laws remain separately owned providers | Common force-coordinate interface for later QSS and structural load paths |
| `EQ-VEH-0008` through `0010` | Explicit road-compatible wheel reduction, reduced quasi-static body equilibrium, and active-contact road-normal reaction recovery | `prototype_authorized` only within `AUTH-VEH-0004` after review and merge | Implement only the explicit provider-neutral contracts; report scaled residual/tangent/contact diagnostics and preserve force-coordinate signs | No hidden WUFR mass defaults, crossweight/load-transfer rules, alternate contact modes, maneuver tire/aero/brake inputs, or WUFR road-reaction claims before separate mass-force review | Generic static equilibrium kernel and synthetic verification backbone |
...
| `EQ-SUSP-0009` through `0012` | Arm-fixed actuation-pickup transport, one-axis rocker closure, ideal coilover displacement, and signed local damper-over-wheel displacement derivative | `prototype_authorized` only within `AUTH-SUSP-0003` | Compose `MOD-SUSP-0001`/`MOD-SUSP-0002`, retain source attachment roles and nominal branch, and preserve structured closure/derivative failures | No constitutive force law inside this kinematic layer, wheel/ride rate, damping, load transfer, compliance, installed limits, packaging, or optimization authority | Fundamental rigid actuation kinematics and downstream geometry-state interface |
| `EQ-SUSP-0013` through `0015` | Explicit spring compression/reference state, conservative force/energy/tangent law, and signed generalized spring force | `prototype_authorized` within `AUTH-SUSP-0004`; implementation merged in PR #48 | Maintain bounded linear, explicitly reviewed affine tangent-rate, and source-defined nonlinear laws through energy/virtual work; preserve seated-spring, domain, and assumption provenance | No unlabeled/invented spring curve, hidden reference inference, `F=k_t*x` substitution for progressive tangent laws, scalar MR substitution, equilibrium, installed limits, or production authority | First elastic suspension-force provider for prescribed-force QSS |
| `EQ-SUSP-0016` through `0018` | Source-defined coupled ARB deformation/Jacobian, conservative energy/action/tangent law, and signed generalized ARB force | `prototype_authorized` through `AUTH-SUSP-0005` to `AUTH-SUSP-0009` | Maintain generic conservative mechanics plus WUFR two-arm `d=[d_L,d_R]`, `F=[k_b d_L,k_b d_R]`, `U=0.5 k_b(d_L^2+d_R^2)`, `Q=-J_d^T F`; settings are discrete `280/300/400/700/2300 N/mm` per arm; preserve rocker and physical-wheel coordinate chains and independent energy-gradient verification | No stiffness interpolation/stacking/rescaling, historical motion-ratio/body-roll/track-width shortcuts, whole-vehicle equilibrium/load-transfer claim, structural release, installed limits, or production authority | Coupled WUFR ARB force provider through physical wheel-center vertical generalized coordinates |
...
## 5. Whole-vehicle force-coordinate and quasi-static equilibrium transition gate
...
Since PR #48 and PR #56, the project now has separately authorized conservative spring and WUFR ARB providers in physical wheel-center vertical coordinates. That closes two constitutive ingredients but does **not** upgrade `MOD-VEH-0003` into an equilibrium solver. Damper/tire/aero/brake/gravity force authority, total/sprung mass force authority, per-corner unsprung allocation, four-corner wheel-load generation, heave/roll/pitch equilibrium and compatibility, alternate contact modes, tire vertical compliance, linkage/member/bearing reactions, stress/FEA, compliance, installed limits, and production optimization remain separate authorization problems.

The measured static corner-weight diagonal split is not reproduced by `MOD-VEH-0003`: rigid-body vertical force/roll/pitch equilibrium alone does not uniquely close four vertical reactions. The next equilibrium gate must explicitly combine the reviewed spring/ARB elastic providers with body/contact coordinates and a compatibility/equilibrium closure; it may not hide a crossweight or load-transfer rule.

`AUTH-VEH-0004` permits that next gate only as a provider-neutral prototype after review and merge:

1. consume an explicit road-compatible physical wheel-coordinate map `z_w(q_b)` and signed `J_wb`, never a track-width/body-roll or scalar motion-ratio substitute;
2. consume conservative suspension wheel generalized forces from separately authorized providers and assemble `R_b=Q_body_ext+J_wb^T Q_susp_w`;
3. solve only the reduced quasi-static body residual with declared coordinate scales/bounds, deterministic tangent/line-search behavior, convergence diagnostics, and structured provider/numerical failures;
4. recover active-contact road reactions afterward from explicit wheel-coordinate equilibrium `Q_susp_i+Q_wheel_ext_i+c_i*lambda_i=0`;
5. retain any negative `lambda_i`, classify wheel lift/contact-mode invalidity, and never clip or redistribute;
6. pass `BENCH-VEH-0005` and `BENCH-VEH-0006`, including the analytical 100 kg/four-10-kN/m synthetic support case and independent conservative energy-gradient checks;
7. keep all synthetic mass values fixture-local and prohibit hidden WUFR mass defaults, historical `10 kg/corner`, `207 kg` sprung, `220+100 kg` total, crossweight, scalar wheel-rate, and scalar ARB substitutions;
8. leave WUFR road reactions unavailable until a separate mass/gravity authorization supplies a reviewed sprung-body gravity input and per-corner wheel external-force allocation.

The PR #58 source audit records the current stopping boundary: reviewer-measured unsprung mass is `10 kg front axle + 10 kg rear axle`, with no reviewed left/right split or application convention. Generic equilibrium-kernel development does not require inventing that WUFR parameter; WUFR four-corner gravity equilibrium does.
...
The WUFR coupled ARB prototype is now controlled by the staged `AUTH-SUSP-0005` through `AUTH-SUSP-0009` chain:

1. `AUTH-SUSP-0005` / PR #50 establishes the generic source-defined conservative ARB coordinate, energy/action/tangent, and generalized-force mechanics with explicit zero reference and structured failure behavior;
2. the corrected WUFR constitutive authority is the five **discrete SolidWorks FEA single-blade-arm transverse tip stiffnesses** `280/300/400/700/2300 N/mm`, converted to `280000/300000/400000/700000/2300000 N/m`; interpolation, averaging, source stacking, and scalar factor-two/half/sqrt(2) rescaling are prohibited;
3. historical `2560/2270 N*m/deg` is retained only as downstream setting-1 effective axle roll-stiffness comparison evidence from the same blade-study lineage, not as governing blade constitutive authority;
4. `AUTH-SUSP-0006` freezes the nominal front/rear WUFR Z-bar point/topology fixture and rigid rocker-pickup transport without inventing a mechanism map;
5. `AUTH-SUSP-0007` resolves the installed ideal blade as two per-arm elastic coordinates with `F=[k_b d_L,k_b d_R]` and `U=0.5 k_b(d_L^2+d_R^2)`;
6. `AUTH-SUSP-0008` / PR #54-#55 authorizes the nominal-continuation two-arm mechanism closure, two-step `J_d=partial[d_L,d_R]/partial[theta_RL,theta_RR]`, and `Q_rocker=-J_d^T F` in left/right rocker-angle coordinates;
7. `AUTH-SUSP-0009` / PR #56 evaluates signed `rho_rw=d(theta_R)/d(delta_z_wc_body)` from the reviewed physical wheel/rocker solvers, forms `J_dz=J_dtheta*diag(rho_rw_L,rho_rw_R)`, and returns `Q_z=-J_dz^T F` in left/right physical wheel-center vertical coordinates;
8. `BENCH-SUSP-0011` through `BENCH-SUSP-0017` preserve the generic constitutive checks, source/fixture gates, two-arm energy semantics, mechanism/Jacobian checks, and independent rocker/wheel energy-gradient verification;
9. body roll, track width, wheel-travel difference, historical scalar motion ratio, reduced-axle back-fitting, hidden absolute-value sign repair, and hidden preload offsets remain prohibited substitutes for the reviewed coordinate chain;
10. `Q_z` is not by itself tire load transfer, body roll, axle roll moment, LLTD, or whole-vehicle equilibrium. Those claims require the next separately reviewed vehicle equilibrium problem.

Physical blade/system correlation, friction/backlash/compliance, installed travel/limits, linkage loads, stress/FEA, packaging, installed/as-built validation, and production optimization remain separate authorization problems.
