...
| `MOD-SUSP-0004` | Conservative coil-spring compression, constitutive force, stored energy, tangent stiffness, and generalized-force provider | `prototype_authorized` only after `AUTH-SUSP-0004` / PR #47 review and merge | After authorization merge, implement `EQ-SUSP-0013` through `0015` and pass `BENCH-SUSP-0009` through `0010`; front WUFR linear law may be exercised only with explicit compression/reference inputs | Rear 30-to-36 N/mm progression versus compression is not yet sourced; WUFR spring-seat/reference mapping from solved coilover length to absolute compression remains unfrozen; damper/ARB/equilibrium/installed authority remains separate | First conservative suspension-force element for later QSS composition |
...
| `EQ-SUSP-0013` through `0015` | Explicit spring compression/reference state, conservative force/energy/tangent law, and signed generalized spring force | `prototype_authorized` only within `AUTH-SUSP-0004` after PR #47 review/merge | Implement conservative linear and source-defined bounded nonlinear laws through energy/virtual work; preserve seated-spring and source-domain failures | No invented rear progressive curve, hidden preload/reference inference, damper/ARB forces, scalar MR substitution, equilibrium, installed limits, or production authority | First elastic suspension-force provider for prescribed-force QSS |
...
1. define positive spring compression from explicit spring free length and spring-seat separation, or an equivalent explicit preload/reference mapping; do not infer an unknown seat offset from nominal coilover length;
2. evaluate a source-defined conservative `F_s(x_s)`, stored energy `U_s`, and local tangent stiffness `k_t` only inside the reviewed constitutive domain;
3. map spring energy into generalized force with `Q_s=-partial U_s/partial q`; for `x_s=x_pre+L_ref-L_d`, use `Q_s=F_s partial L_d/partial q` and preserve the sign of `rho_dw` for the physical wheel coordinate;
4. treat negative compression as an explicit `spring_unseated`/contact-mode failure rather than clipping;
5. permit the reviewed WUFR front 36 N/mm linear law as a function of explicit spring compression, while returning `progressive_law_incomplete` for the rear 30-to-36 N/mm spring until its progression versus compression is sourced;
6. prohibit endpoint averaging, damper-stroke progression assumptions, historical OptimumK Motion Ratio substitution, and scalar `k*MR^2` as governing force physics;
7. pass `BENCH-SUSP-0009` and `BENCH-SUSP-0010` before implementation is considered complete.

Damper velocity force, gas/friction/hysteresis/stops, anti-roll-bar coupling, tire constitutive behavior, load transfer/equilibrium, compliance, physical joint/installed travel limits, linkage loads, stress/FEA, packaging, installed/as-built validation, and production optimization remain separate authorization problems.
