...
| `MOD-SUSP-0005` | Coupled conservative anti-roll-bar bilateral deformation, energy/action, tangent stiffness, and generalized-force provider | `prototype_authorized` only after `AUTH-SUSP-0005` / PR #49 review and merge | After authorization merge, implement `EQ-SUSP-0016` through `0018` and pass `BENCH-SUSP-0011` through `0012`; generic/synthetic mechanics plus WUFR geometry/reference adaptation may proceed | WUFR front/rear blade or assembled-Z-bar constitutive stiffness authority is still missing; numeric WUFR ARB force/energy must remain unavailable until traceable force-deflection/torque-angle evidence is frozen | Coupled ARB elastic provider for later prescribed-force QSS composition |
...
| `EQ-SUSP-0016` through `0018` | Source-defined bilateral ARB deformation/Jacobian, conservative energy/action/tangent law, and signed generalized ARB force | `prototype_authorized` only within `AUTH-SUSP-0005` after PR #49 review/merge | Implement bilateral mechanism state and generic/source-authorized constitutive laws through `Q_ARB=-J_s^T a_ARB`; preserve zero-preload/configuration/source provenance and explicit no-bar state | No invented WUFR stiffness, sketch-row topology inference, scalar ARB wheel-rate substitution, equilibrium, linkage loads, installed limits, or production authority | Coupled anti-roll-bar force provider for prescribed-force QSS |
...
Physical rear spring testing and calibrated perch/seat or shock-pot reference evidence are replacement/validation gates for `ASM-SUSP-0002`; they do not block the explicitly assumption-bounded spring prototype.

`AUTH-SUSP-0005`, after PR #49 review and merge, permits only the first coupled anti-roll-bar mechanics prototype:

1. treat the ARB as one bilateral left/right elastic element; do not replace it with two independent corner springs or a governing scalar `k_ARB*MR^2` wheel-rate shortcut;
2. reconstruct a source-defined signed deformation coordinate/vector `s_ARB(q_left,q_right)` and Jacobian `J_s=partial(s_ARB)/partial(q)` from explicit bilateral mechanism geometry/topology rather than exported sketch-point row order;
3. evaluate conservative `U_ARB(s)`, conjugate action `a_ARB=gradient_s(U_ARB)`, and local tangent stiffness/matrix only where constitutive authority exists;
4. map the elastic action into generalized coordinates with `Q_ARB=-J_s^T*a_ARB`, preserving left/right signs and coordinate order;
5. use `ASM-SUSP-0003` only as explicit WUFR-27 design-intent authority for the 2025/current Z-bar carryover and zero intentional preload reference; do not promote it to installed/as-built authority;
6. use current populated WUFR-26 suspension/front/rear ARB CAD and drawings as carryover geometry evidence until a populated WUFR-27 revision supersedes them; current direct WUFR-27 A0303/A0305 assembly files are byte-identical placeholders/copies and are not independent geometry authority;
7. retain WUFR-26 FSA front-active/rear-suppressed ARB state as configuration evidence only; a later vehicle configuration must explicitly select enabled/disabled state, with an explicit no-bar branch returning zero action/energy/generalized force;
8. keep WUFR-specific constitutive force/energy unavailable as `missing_stiffness_authority` until traceable front/rear blade or assembled-Z-bar force-deflection/torque-angle evidence is frozen;
9. reject `Weight_transfer_sensitivity.m` literals `2560/2270`, spec-sheet suspension roll rates `556/458 Nm/deg`, blade material/dimensions alone, and unrecovered SolidWorks Simulation databases as constitutive stiffness substitutes;
10. pass `BENCH-SUSP-0011` and `BENCH-SUSP-0012` before implementation is considered complete.

Physical rear spring testing and calibrated spring-seat/shock-pot evidence remain spring replacement/validation gates. Traceable ARB stiffness evidence remains the gate for numeric WUFR anti-roll-bar force/energy. Damper velocity force, gas/friction/hysteresis/stops, tire constitutive behavior, load transfer/equilibrium, compliance outside the separately authorized elastic coordinates, physical joint/installed travel limits, linkage loads, stress/FEA release, packaging, installed/as-built validation, and production optimization remain separate authorization problems.
