...
8. permit the separately reviewed PR #46 WUFR design-intent adapter because it freezes explicit CAD axle/track references, named scale-state planar CG evidence, separately sourced no-driver CG height, source/configuration identity, and authority boundaries;
9. keep the no-driver composite CG explicitly source-separated and leave the driver/no-fuel `z_CG` unavailable;
10. retain ideal two-force-member linkage statics as a downstream fidelity decision only, with no linkage-force or stress output in `MOD-VEH-0003`;
11. maintain `BENCH-VEH-0003` and `BENCH-VEH-0004` as implementation gates.

Spring/damper/ARB/tire/aero/brake/gravity constitutive forces, sprung/unsprung mass force authority, four-corner wheel-load generation, heave/roll/pitch equilibrium, alternate contact modes, tire vertical compliance, linkage/member/bearing reactions, stress/FEA, compliance, installed limits, and production optimization remain separate authorization problems.
