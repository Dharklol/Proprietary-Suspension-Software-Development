...
| `MOD-VEH-0003` | Whole-vehicle body/road point transport, wrench assembly, virtual-work generalized-force mapping, and rigid four-contact classification | `prototype_authorized` only after `AUTH-VEH-0003` / PR #45 review and merge | Implement `EQ-VEH-0004` through `0007` with `BENCH-VEH-0003` through `0004`; synthetic mechanics first and explicit unavailable WUFR placement | Common body/CG origin, axle/contact placement, mass properties, force laws, equilibrium, linkage loads, installed evidence, and production authority remain separate | Mechanics foundation for later spring/ARB providers, prescribed-force QSS, and ideal linkage statics |
...
| `EQ-VEH-0004` through `0007` | Body-fixed point transport, point-force wrench translation, virtual-work generalized force, and rigid flat-road contact admissibility | `prototype_authorized` only within `AUTH-VEH-0003` after PR #45 merge | Implement exact frame/origin/application-point contracts, analytical mechanics benchmarks, and structured contact/Jacobian failures | No constitutive force law, equilibrium solve, negative-load clipping, tire vertical compliance, alternate contact mode, linkage forces, stress, or installed authority | Common force-coordinate interface for later QSS and structural load paths |
...
`AUTH-VEH-0003`, after PR #45 review and merge, permits only the bounded common mechanics foundation:

1. preserve one explicit right-handed whole-vehicle body frame and one explicit road/inertial frame;
...
4. map applied actions into signed generalized forces with `EQ-VEH-0006` and declared coordinate order, units, Jacobian method, steps, and residuals;
5. classify only the flat-road, vertically rigid, all-four-active contact mode with `EQ-VEH-0007`;
6. report any negative normal reaction as wheel lift/contact-mode invalidity and never clip it;
7. retain WUFR-27 inheritance of WUFR-26 suspension geometry without treating wheelbase as a common front/rear/CG origin transform;
8. retain ideal two-force-member linkage statics as a downstream fidelity decision only, with no linkage-force or stress output in `MOD-VEH-0003`;
9. pass `BENCH-VEH-0003` and `BENCH-VEH-0004` before implementation merge.

Spring/damper/ARB/tire constitutive forces, mass/CG and wheel-load generation, heave/roll/pitch equilibrium, alternate contact modes, tire vertical compliance, linkage/member/bearing reactions, stress/FEA, compliance, installed limits, and production optimization remain separate authorization problems.
